...
function writeSessionLogFile(session: BotLogSession, stopSource: "f2" | "ui", stopReason: string): void {
  try {
...
export function startAutomateBotLogSession(botId: string, source: "f2" | "ui"): void {
  if (currentSession) {
...
export function stopAutomateBotLogSession(source: "f2" | "ui", reason: string = "stopped"): void {
  if (!currentSession) {
    return;
  }
