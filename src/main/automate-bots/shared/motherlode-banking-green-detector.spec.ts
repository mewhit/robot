...
} from "./motherlode-banking-green-detector";
...
function validateBestBox(screenshotPath: string, best: MotherlodeBankingGreenBox): boolean {
  ...
  if (!expected.shouldDetect) {
    console.error(`Expected no banking green detection for ${basename}, but got one.`);
    return false;
  }
...
async function testDetection(screenshotPath: string): Promise<boolean> {
  ...
  const debugPath = path.join(debugOutputDir, `${basenameNoExt}-motherlode-banking-green-boxes.png`);

  const boxes = detectMotherlodeBankingGreenBoxesInScreenshot(bitmap);
  if (boxes.length === 0) {
    saveBitmapWithMotherlodeBankingGreenBoxes(bitmap, [], debugPath);
    console.log(`Debug image: ${debugPath}`);

    if (expected && !expected.shouldDetect) {
      console.log("No motherlode banking green boxes detected (expected).\n");
      return true;
    }

    console.log("No motherlode banking green boxes detected.");
    return false;
  }

  const best = boxes[0];
  console.log(
    `Best green-box at (${best.x}, ${best.y}) ${best.width}x${best.height} center=(${best.centerX}, ${best.centerY}) pixels=${best.pixelCount} fill=${best.fillRatio.toFixed(3)} green-strength=${best.greenStrength.toFixed(1)} score=${best.score.toFixed(1)}`,
  );

  saveBitmapWithMotherlodeBankingGreenBoxes(bitmap, [best], debugPath);
  console.log(`Debug image: ${debugPath}`);
...
async function main(): Promise<void> {
  ...
  const screenshots = args.length > 0 ? args : ["test-images/motherlode-banking-green/*.png"];
  const expandedScreenshots = expandScreenshotArgs(screenshots);

  console.log("\nMotherlode Banking Green Detector Test Suite");
