...
function buildCyanMask(bitmap: RobotBitmap): Uint8Array {
...
function toCyanBox(candidate: BoxCandidate): CyanBox | null {
  ...
  const score = candidate.pixelCount + height * 6 + width * 2 + fillRatio * 180 - Math.abs(aspectRatio - 2.1) * 35;
...
function sortBoxes(boxes: CyanBox[]): CyanBox[] {
...
export function detectCyanBoxesInScreenshot(bitmap: RobotBitmap): CyanBox[] {
  const mask = buildCyanMask(bitmap);
  ...
  return sortBoxes(mergedComponents.map(toCyanBox).filter((box): box is CyanBox => box !== null));
}

export function detectBestCyanBoxInScreenshot(bitmap: RobotBitmap): CyanBox | null {
  return detectCyanBoxesInScreenshot(bitmap)[0] ?? null;
}

export function saveBitmapWithCyanBoxes(bitmap: RobotBitmap, boxes: CyanBox[], filename: string): void {
  const png = new PNG({
    width: bitmap.width,
