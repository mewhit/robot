...
} from "./motherlode-banking-yellow-detector";
...
function validateBestBox(screenshotPath: string, best: MotherlodeBankingYellowBox): boolean {
  ...
  if (!expected.shouldDetect) {
    console.error(`Expected no banking yellow detection for ${basename}, but got one.`);
    return false;
  }
...
async function testDetection(screenshotPath: string): Promise<boolean> {
  ...
  const debugPath = path.join(debugOutputDir, `${basenameNoExt}-motherlode-banking-yellow-boxes.png`);

  const boxes = detectMotherlodeBankingYellowBoxesInScreenshot(bitmap);
  if (boxes.length === 0) {
    saveBitmapWithMotherlodeBankingYellowBoxes(bitmap, [], debugPath);
    console.log(`Debug image: ${debugPath}`);

    if (expected && !expected.shouldDetect) {
      console.log("No motherlode banking yellow boxes detected (expected).\n");
      return true;
    }

    console.log("No motherlode banking yellow boxes detected.");
    return false;
  }

  const best = boxes[0];
  console.log(
    `Best yellow-box at (${best.x}, ${best.y}) ${best.width}x${best.height} center=(${best.centerX}, ${best.centerY}) pixels=${best.pixelCount} fill=${best.fillRatio.toFixed(3)} yellow-strength=${best.yellowStrength.toFixed(1)} score=${best.score.toFixed(1)}`,
  );

  saveBitmapWithMotherlodeBankingYellowBoxes(bitmap, [best], debugPath);
  console.log(`Debug image: ${debugPath}`);
...
async function main(): Promise<void> {
  ...
  const screenshots = args.length > 0 ? args : ["test-images/motherlode-banking-yellow/*.png"];
  const expandedScreenshots = expandScreenshotArgs(screenshots);

  console.log("\nMotherlode Banking Yellow Detector Test Suite");
