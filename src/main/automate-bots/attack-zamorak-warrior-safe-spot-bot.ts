...
async function runLoop(window: Window): Promise<void> {
  ...
  try {
    ...
    if (!startupNpcBox) {
      const message =
        `${BOT_NAME} startup check failed. Missing the cyan NPC outline. Fix: enable RuneLite NPC Indicators, tag Zamorak Warrior, and keep one visible on screen.`;
      errorWithDelta(`Automate Bot (${BOT_NAME}): ${message}`);
      notifyUserAndStop(message);
      return;
    }
    ...
    while (AppState.automateBotRunning) {
      ...
      try {
        ...
        logWithDelta(`Automate Bot (${BOT_NAME}): loop #${loopIndex} - clicking target at (${clickPoint.x}, ${clickPoint.y}).`);
